    ...
    for (line_index, line) in markdown.lines().enumerate() {
        ...
        if let Some((label, value)) = labeled_bullet(line) {
            match label.as_str() {
                ...
                "evidence" | "implementation evidence (independent verification pending)" => {
...
mod tests {
    ...
    fn evidence_metadata_is_non_topology_and_unknown_labels_stay_rejected() {
        ...
        let pending_verification = format!(
            "{}   - **Implementation evidence (independent verification pending):** formatter passed\n",
            block(1, true, "SEQ", "Foundation", "A", "none")
        );
        let parsed = parse_numbered_prd(&pending_verification).unwrap();
        assert_eq!(parsed[0].evidence.as_deref(), Some("formatter passed"));
